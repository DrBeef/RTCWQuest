...
long RllDecodeMonoToMono( unsigned char *from,short *to,unsigned int size,char signedOutput,unsigned short flag ) {
	...
	for ( z = 0; z < size; z++ ) {
		prev = to[z] = (signed short)( prev + cin.sqrTable[from[z]] );
	}
...
long RllDecodeMonoToStereo( unsigned char *from,short *to,unsigned int size,char signedOutput,unsigned short flag ) {
	...
	for ( z = 0; z < size; z++ ) {
		...
		to[z * 2 + 0] = to[z * 2 + 1] = (signed short)( prev );
	}
...
long RllDecodeStereoToStereo( unsigned char *from,short *to,unsigned int size,char signedOutput, unsigned short flag ) {
	...
	signed short prevL, prevR;     //DAJ was int
	...
	for ( z = 0; z < size; z += 2 ) {
		...
		to[z + 0] = (signed short)( prevL );
		to[z + 1] = (signed short)( prevR );
	}
...
long RllDecodeStereoToMono( unsigned char *from,short *to,unsigned int size,char signedOutput, unsigned short flag ) {
	...
	for ( z = 0; z < size; z += 1 ) {
		...
		to[z] = (signed short)( ( prevL + prevR ) / 2 );
	}
...
static void readQuadInfo( byte *qData ) {
	...
	cinTable[currentHandle].t[0] = ( 0 - (signed int)cin.linbuf ) + (unsigned int)cin.linbuf + cinTable[currentHandle].screenDelta;
	cinTable[currentHandle].t[1] = ( 0 - ( (signed int)cin.linbuf + cinTable[currentHandle].screenDelta ) ) + (unsigned int)cin.linbuf;
...
//Com_Printf( "SCR_AdjustFrom640 before x=%f, y=%f, w=%f, h=%f\n", x,y,w,h );
	SCR_AdjustFrom640( &x, &y, &w, &h );
//Com_Printf( "SCR_AdjustFrom640 x=%f, y=%f, w=%f, h=%f\n", x,y,w,h );
...
//Com_Printf( "letterBox, barheight=%f\n", barheight );
...
//Com_Printf( "re.DrawStretchRaw(%f, %f, %f, %f, %i, %i, %p, %i, %i )\n", x, y, w, h, cinTable[handle].drawX, cinTable[handle].drawY, buf, handle, cinTable[handle].dirty );
//Com_Printf( "premier point %x )\n", *(unsigned int*)buf );
