...
void glBox( idVec3 &color, idVec3 &point, float size ) {
	...
	maxs[2] += size;
	#ifdef HAVE_GLES
	//*TODO Add the function (easy but boring)
	#else
	qglColor3fv( color );
...
void idSplineList::draw( bool editMode ) {
	...
	if (glcol)
		qglDisableClientState( GL_COLOR_ARRAY );
	qglVertexPointer (3, GL_FLOAT, 0, *controlPoints[0]);
	qglDrawArrays(GL_POINTS, 0, controlPoints.Num());				
	#else
	qglBegin( GL_POINTS );
	for ( i = 0; i < controlPoints.Num(); i++ ) {
	...
	#ifdef HAVE_GLES
	qglVertexPointer (3, GL_FLOAT, 0, *splinePoints[0]);
	qglDrawArrays(GL_LINE_STRIP, 0, count );				
	#else
	...
	if ( editMode ) {
		...
		#ifdef HAVE_GLES
		qglVertexPointer (3, GL_FLOAT, 0, *splinePoints[0]);
		qglDrawArrays(GL_POINTS, 0, splinePoints.Num());				
		#else
		qglBegin( GL_POINTS );
		for ( i = 0; i < count; i++ ) {
			qglVertex3fv( *splinePoints[i] );
